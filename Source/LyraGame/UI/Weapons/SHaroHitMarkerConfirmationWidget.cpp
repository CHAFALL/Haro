...
SHaroHitMarkerConfirmationWidget::SHaroHitMarkerConfirmationWidget()
{
}

void SHaroHitMarkerConfirmationWidget::Construct(const FArguments& InArgs, const FLocalPlayerContext& InContext, const TMap<FGameplayTag, FSlateBrush>& ZoneOverrideImages)
{
...
int32 SHaroHitMarkerConfirmationWidget::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	...
	if (bDrawMarkers)
	{
		...
		TArray<FHaroScreenSpaceHitLocation> LastWeaponDamageScreenLocations;
		if (APlayerController* PC = MyContext.IsInitialized() ? MyContext.GetPlayerController() : nullptr)
		{
			if (UHaroWeaponStateComponent* WeaponStateComponent = PC->FindComponentByClass<UHaroWeaponStateComponent>())
			{
		...
		if ((LastWeaponDamageScreenLocations.Num() > 0) && (PerHitMarkerImage != nullptr))
		{
			...
			for (const FHaroScreenSpaceHitLocation& Hit : LastWeaponDamageScreenLocations)
			{
...
FVector2D SHaroHitMarkerConfirmationWidget::ComputeDesiredSize(float) const
{
	return FVector2D(100.0f, 100.0f);
}

void SHaroHitMarkerConfirmationWidget::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	HitNotifyOpacity = 0.0f;

	if (APlayerController* PC = MyContext.IsInitialized() ? MyContext.GetPlayerController() : nullptr)
	{
		if (UHaroWeaponStateComponent* DamageMarkerComponent = PC->FindComponentByClass<UHaroWeaponStateComponent>())
		{
